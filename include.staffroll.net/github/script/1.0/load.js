...
StaffRoll.Credit = function(){
	...
	this.callTheme = function(){
		...
		for (var i=0; i<scripts.length; i++){
			if (scripts[i].src && scripts[i].src.match(/^https?:\/\/include\.staffroll\.(net|site)\/g\/script\/1\.0\/load\.js(\?.*)?$/)){
				script = scripts[i];
			}
		}
		...
		if (theme != 'default'){
			...
			script = document.createElement('script');
				...
				script.setAttribute('src', 'http://include.'+StaffRoll.net.replace('beta.','')+'/g/theme/'+theme+'/1.0/main.js');
			link = document.createElement('link');
				link.setAttribute('rel', 'stylesheet');
				link.setAttribute('type', 'text/css');
				link.setAttribute('href', 'http://include.'+StaffRoll.net.replace('beta.','')+'/g/theme/'+theme+'/1.0/main.css');
